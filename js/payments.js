      ...
      evt.preventDefault();
        ...
        stripe.createPaymentMethod({
            ...
            if (result.error) {
                ...
                createCustomer(result.paymentMethod.id, cardholderEmail, stripe, stripeKey);
            }
        });
    });
...
async function getPaymentIntent(stripe, ev, total) {
    return fetch('https://app.criystal.com/web/client/getParam', {
        ...
        then(clientSecret => {
            console.log('pt obtained ' + clientSecret);
...
async function createCustomer(paymentMethod, cardholderEmail, stripe, stripeKey) {
    return fetch('https://app.criystal.com/web/client/getParam', {
        method: 'get',
    }).then(response => {
        if (response.ok) {
        ...
        return fetch('https://app.criystal.com/web/subscription/create-customer', {
            method: 'post',
            headers: {
                'Content-Type': 'application/json'
            },
            ...
            if (response.ok) { 
                ...
                response.json().then(function(data) {
                    handleSubscription(data, stripe);
                });
            } else {
                showCardError('We are sorry we could not process this payment method');
            }
...
function handleSubscription(subscription, stripe) {
    if (subscription.subscriptionState === 'trialing' || subscription.subscriptionState === 'active') {
        orderComplete();
    } else {
        stripe.confirmCardPayment(subscription.paymentIntentSecret).then(function(result) {
            if (result.error) {
                ...
                // TODO confirmSubscription(subscription.id);
                changeLoadingState(false);
                orderComplete();
            }
        });
